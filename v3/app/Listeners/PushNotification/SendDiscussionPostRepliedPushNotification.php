...
class SendDiscussionPostRepliedPushNotification
{
    public function __invoke(DiscussionPostReplied $event): void
    {
        try {
            ...
            $data = $service->buildPushData(
                ...
                [
                    ...
                    'event_key' => sprintf('discussion_post_replied:post:%d', $event->postId),
                ]
            );
            ...
            $service->sendPushToRecipient(
                $recipient,
                'New Reply',
                sprintf('%s replied to your comment', $actorName),
                $data
            );
        } catch (\Throwable) {
            return;
