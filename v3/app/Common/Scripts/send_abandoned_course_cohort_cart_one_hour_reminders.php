...
$rows = $service->getAbandonedCartReminderCandidatesAfterMinutes(60);

foreach ($rows as $row) {
    EventDispatcher::dispatch(new AbandonedCourseCohortCart((int) $row['id'], 'one_hour'));
}

echo sprintf("Dispatched one-hour abandoned course cohort cart reminders: %d\n", count($rows));
